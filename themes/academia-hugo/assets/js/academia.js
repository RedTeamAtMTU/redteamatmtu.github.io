...
(function ($) {
  ...
  function initMap() {
    if ($('#map').length) {
      ...
      if (map_provider == 1) {
        ...
        marker.bindPopup(address + '<p><a href="https://www.openstreetmap.org/directions?engine=osrm_foot&route=' + url + '">Directions via OpenStreetMap</a></p>');
      }
    }
  }

  /* ---------------------------------------------------------------------------
   * Meeting countdown on "meetings.html" widget.
   * --------------------------------------------------------------------------- */

  function initCountdown() {
    if ($('#meeting-countdown').length) {
      let upcomingMeetingsStr = document.getElementById('upcoming-meetings').value;
      let meetingTime = $('#meeting-time').val();
      let meetingLength = $('#meeting-length').val();

      //console.log('Meeting time:', meetingTime);
      //console.log('Upcoming meetings:', upcomingMeetingsStr);
      //console.log('Meeting length:', meetingLength);

      const upcomingMeetings = upcomingMeetingsStr ? upcomingMeetingsStr.split(',') : [];

      function findNextMeeting() {
        const now = new Date();

        for (const dateStr of upcomingMeetings) {
          const [year, month, day] = dateStr.split('-').map(Number);
          const [hours, minutes] = meetingTime.split(':').map(Number);
          //const meetingStart = new Date(Date.UTC(year, month - 1, day, hours - 5, minutes)); // ET is UTC-5
          const meetingStart = new Date(Date.UTC(year, month - 1, day, hours + 4, minutes));
          const meetingEnd = new Date(meetingStart.getTime() + meetingLength * 60000);
          if (now.getTime() >= meetingStart.getTime() && now.getTime() <= meetingEnd.getTime()) {
            return { start: meetingStart, end: meetingEnd, isHappeningNow: true };
          }
          if (meetingStart.getTime() > now.getTime()) {
            return { start: meetingStart, end: meetingEnd, isHappeningNow: false };
          }
      ...
      function getTimeRemaining(endtime) {
        ...
        if (diff <= 0) return { total: 0, months: 0, days: 0, hours: 0, minutes: 0, seconds: 0 };
        ...
        const remainingDiff = endtime - tempDate;
      ...
      function updateCountdown() {
        ...
        if (!nextMeeting) {
          countdownElement.innerHTML = "<div class='countdown-container'><div class='countdown-label'>No upcoming meetings scheduled</div></div>";
          return;
        }

        if (nextMeeting.isHappeningNow) {
          const t = getTimeRemaining(nextMeeting.end);
          countdownElement.innerHTML = `
            <div class='countdown-container'>
              <div class='countdown-label'>Meeting is happening now!</div>
              <div class='countdown-time'>Ends in: ${t.hours}h ${t.minutes}m ${t.seconds}s</div>
            </div>
          `;
          return;
        }

        const t = getTimeRemaining(nextMeeting.start);
        let countdownStr = `
          <div class="countdown-container">
        ...
        if (t.months > 0) countdownStr += `${t.months}mo `;
        if (t.days > 0) countdownStr += `${t.days}d `;
        if (t.hours > 0) countdownStr += `${t.hours}h `;
        if (t.minutes > 0) countdownStr += `${t.minutes}m `;
        if (t.seconds > 0) countdownStr += `${t.seconds}s`;
        //countdownStr += `${t.hours}h ${t.minutes}m ${t.seconds}s`;
        countdownStr += `</div></div>`;
  ...
  $(window).on('load', function () {
    ...
    // Initialise countdown timer if necessary.
    initCountdown();
